...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingAuthUser) {
      console.log('👤 Utilisateur existe déjà dans auth:', existingAuthUser.id);
      ...
      if (syncError) {
        console.error('⚠️ Erreur sync ID:', syncError.message);
      }
      ...
      console.log('👤 Utilisateur manque dans auth, création...');
      ...
      return new Response(
        JSON.stringify({ 
          success: true,
          message: 'Utilisateur auth créé et synchronisé',
